...
async function Youtube(Response, infos) {
    try {
        ...
        if (Retry > 0) {
            Retry--
            console.log(`\x1b[37m --==--  \x1b[31m${translation[settings.language].Error.Retry}\x1b[37m --==-- \x1b[0m`)
            await WaitSeconds(3)
            console.clear()
            await WaitSeconds(0.5)
            this.DownloadSounds({ format, url })
        } else {
            console.log(`\x1b[37m --==--  \x1b[31m${translation[settings.language].Error.MaxRetry}\x1b[37m --==-- \x1b[0m`)
            await WaitSeconds(5)
            Retry = 3
            console.clear()
        }
